...
def set_user_id(user_id, full_name, admin):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO myusers (user_id, full_name, admin) VALUES (%s,%s,%s)", 
...
def is_user_exists(user_id):
    ...
    try:
        ...
        cursor = conn.cursor()
        query = (f"SELECT * FROM myusers WHERE user_id='{user_id}'")
...
def get_all_users(status):
    ...
    try:
        conn = MySQLConnection(**db_config)
        cursor = conn.cursor()
        cursor.execute(f"SELECT user_id FROM myusers WHERE admin='{status}'")
...
def set_admin(user_id, status):
    ...
    try:
        ...
        cursor = conn.cursor()  # открывем соединение
        cursor.execute(f"UPDATE myusers SET admin='{status}' WHERE user_id='{user_id}'")
        conn.commit()  # Подтверждаем изменения
        cursor.close()  # Закрываем курсор
        conn.close()  # Закрываем соединение
